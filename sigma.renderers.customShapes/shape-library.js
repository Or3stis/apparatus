;(function (undefined) {
  ...
  var starShape = function (node, x, y, size, context) {
    ...
    let inRatio = (node.star && node.star.innerRatio) || 0.5
    let outR = size
    let inR = size * inRatio
    let angleOffset = Math.PI / pcount
  ...
  var drawNew = function (node, x, y, size, color, context) {
    ...
    // context.arc(x, y, size / 2, 0, Math.PI, false)
    // // context.arc(x, y, size, 1.25 * Math.PI, 0, false)
    // // context.arc(x, y, size, 0, 0.75 * Math.PI, false)
    // // context.lineTo(x, y)
    // context.closePath()
